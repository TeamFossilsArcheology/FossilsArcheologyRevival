...
public class CultureVatCategory implements DisplayCategory<CultureVatDisplay> {
    ...
    private static final ResourceLocation DARK_TEXTURE = new ResourceLocation(Fossil.MOD_ID, "textures/gui/culture_vat.png");
    ...
    public Component getTitle() {
        return new TranslatableComponent("category.fossil.rei.cultivate");
    }
    ...
    public List<Widget> setupDisplay(CultureVatDisplay display, Rectangle bounds) {
        ...
        widgets.add(Widgets.createDrawableWidget((helper, matrices, mouseX, mouseY, delta) -> {
            ...
            helper.blit(matrices, startPoint.x + 38, startPoint.y + 6, 176, FUEL_HEIGHT, width, PROGRESS_HEIGHT);
        }));
        widgets.add(Widgets.createSlot(new Point(startPoint.x + 4, startPoint.y + 4)).entries(display.getInputEntries().get(0)).disableBackground().markInput());
        ...
        widgets.add(Widgets.createSlot(new Point(startPoint.x + 74, startPoint.y + 4)).entries(display.getOutputEntries().get(0)).disableBackground().markOutput());
        return widgets;
    }
}
