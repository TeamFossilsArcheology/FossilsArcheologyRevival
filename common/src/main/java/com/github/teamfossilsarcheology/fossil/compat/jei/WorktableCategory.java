...
public class WorktableCategory extends WithFuelCategory<WorktableRecipe> {
    ...
    @Override
    public void draw(@NotNull WorktableRecipe recipe, @NotNull IRecipeSlotsView recipeSlotsView, @NotNull PoseStack poseStack, double mouseX, double mouseY) {
        animatedFuel.draw(poseStack, 41, 20);
        animatedProgress.draw(poseStack, 36, 4);
    }

    @Override
    public @NotNull Component getTitle() {
        return new TranslatableComponent("emi.category.fossil.worktable");
    }
    ...
    @Override
    public Class<? extends WorktableRecipe> getRecipeClass() {
        return WorktableRecipe.class;
    }
}
