...
public class CultureVatCategory implements DisplayCategory<CultureVatDisplay> {
    ...
    public List<Widget> setupDisplay(CultureVatDisplay display, Rectangle bounds) {
        ...
        widgets.add(Widgets.createDrawableWidget((helper, matrices, mouseX, mouseY, delta) -> {
            ...
            helper.blit(matrices, startPoint.x + 42, startPoint.y + 21 + FUEL_HEIGHT - progress, 176, FUEL_HEIGHT - progress, FUEL_WIDTH, progress);
            progress = Mth.ceil(time / 250d % (double) PROGRESS_WIDTH);
            helper.blit(matrices, startPoint.x + 38, startPoint.y + 6, 176, FUEL_HEIGHT, progress, PROGRESS_HEIGHT);
        }));
