...
public class FoodMappingsManager extends ResourceLoader<FoodMappingsManager.MapPair> {
    ...
    public FoodMappingsManager() {
        super(PackType.SERVER_DATA, FossilMod.MOD_ID, "food", "json");
    }
                        ...
                        if (jsonElement != null) {
                            ...
                            if (root.has("blocks")) {
                                ...
                                for (JsonElement entry : entries) {
                                    ...
                                    if (object.has("block")) {
                                        Registry.BLOCK.getOptional(new ResourceLocation(object.get("block").getAsString())).ifPresent(block -> {
                                            if (object.has("value")) {
                                        ...
                                        if (object.has("value")) {
                                            blockTags.put(TagKey.create(Registry.BLOCK_REGISTRY, new ResourceLocation(object.get("tag").getAsString())), object.get("value").getAsInt());
                                        } else {
                                            blockTags.put(TagKey.create(Registry.BLOCK_REGISTRY, new ResourceLocation(object.get("tag").getAsString())), -1);
                                        }
                                    }
                                }
                            }
                            if (root.has("items")) {
                                ...
                                for (JsonElement entry : entries) {
                                    ...
                                    if (object.has("item")) {
                                        ...
                                        if (object.has("value")) {
                                            itemTags.put(TagKey.create(Registry.ITEM_REGISTRY, new ResourceLocation(object.get("tag").getAsString())), object.get("value").getAsInt());
                                        } else {
                                            itemTags.put(TagKey.create(Registry.ITEM_REGISTRY, new ResourceLocation(object.get("tag").getAsString())), -1);
                                        }
                                    }
                                }
                            }
    ...
    private void loadTags() {
        ...
        for (FoodType type : FoodType.values()) {
            ...
            for (Map.Entry<TagKey<Block>, Integer> entry : blockTags.get(type).entrySet()) {
                for (Holder<Block> itemHolder : Registry.BLOCK.getTagOrEmpty(entry.getKey())) {
            ...
            for (Map.Entry<TagKey<Item>, Integer> entry : itemTags.get(type).entrySet()) {
                for (Holder<Item> itemHolder : Registry.ITEM.getTagOrEmpty(entry.getKey())) {
    ...
    protected record MapPair(Map<FoodType, Map<Block, Integer>> blocks, Map<FoodType, Map<Item, Integer>> items, Map<FoodType, Map<EntityType<?>, Integer>> entities,
                             Set<EntityType<?>> allEntities, Map<FoodType, Map<TagKey<Block>, Integer>> blockTags, Map<FoodType, Map<TagKey<Item>, Integer>> itemTags,
