    ...
    static {
        ...
        RegistryEntryAddedCallback.event(Registry.BLOCK).register((rawId, id, object) -> {
            if (id.equals(ModBlocks.STRIPPED_CORDAITES_LOG.getId())) {
                if (ModBlocks.CORDAITES_LOG.isPresent())
                    StrippableBlockRegistry.register(ModBlocks.CORDAITES_LOG.get(), object);
            } else if (id.equals(ModBlocks.CORDAITES_LOG.getId())) {
                if (ModBlocks.STRIPPED_CORDAITES_WOOD.isPresent())
                    StrippableBlockRegistry.register(object, ModBlocks.STRIPPED_CORDAITES_WOOD.get());
            } else if (id.equals(ModBlocks.STRIPPED_CORDAITES_WOOD.getId())) {
                if (ModBlocks.CORDAITES_WOOD.isPresent())
                    StrippableBlockRegistry.register(ModBlocks.CORDAITES_WOOD.get(), object);
