...
public class ModItemProvider extends ItemModelProvider {
    ...
    public void javelinItem(JavelinItem item) {
        ResourceLocation texture = FossilMod.location("item/" + item.getRegistryName().getPath());
        builderExisting(texture, item.getRegistryName(), FossilMod.location("item/javelin"));
    }
